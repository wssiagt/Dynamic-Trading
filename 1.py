...
def get_orders(buyprice,profitrate):
    ...
    for x in orders:
        if not x.Id:
            if x.Offset == 0 and x.Price == buyprice and current_profitrate < -0.05:
                return 1
            elif x.Offset == 0 and x.Price == buyprice and current_profitrate + 0.05 < -0.02:
                exchange.CancelOrder(x.Id)
                return 2
            elif x.Offset == 0 and x.Price == buyprice and current_profitrate > -0.05:
                exchange.CancelOrder(x.Id)
                return 2
        else:
            if x.Offset == 0 and x.Price == buyprice and current_profitrate < -0.05:
...
def get_orders2(buyprice,profitrate):
    price = 0
    exchange.SetContractType(contract)
    current_id = '0'
    orders = exchange.GetOrders()
...
def diff_condition(amount, iniamount, profitrate):
    ...
    temp_var = get_orders2(buyprice,profitrate)
    while temp_var != 1:
        if temp_var == 0:
            iniask, inibid, buyprice, sellprice, tradeamount = depth(iniamount)
